...
import { Moon, Sun, Languages, Menu, X, LogOut, Bell, Settings, Calendar, Compass, Sparkles, Globe } from 'lucide-react';
        ...
        <nav className="hidden md:flex items-center gap-6">
          <div className="flex items-center gap-1">
            ...
            <Link 
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors group"
            >
              <Globe size={18} className="text-slate-500 dark:text-slate-400 group-hover:text-blue-500 transition-colors" />
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                {lang === 'ka' ? 'სხვა საიტები' : 'Other Sites'}
              </span>
            </Link>
            ...
            <div className="flex flex-col p-4 gap-4">
              ...
              <motion.div whileTap={{ scale: 0.98 }}>
                <Link 
                  href="/plans"
                  onClick={() => setIsMenuOpen(false)}
                  className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800 font-bold mb-1"
                >
                  <Globe size={18} className="text-blue-500" /> {lang === 'ka' ? 'სხვა საიტები' : 'Other Sites'}
                </Link>
              </motion.div>
